...
for station in stations:
    ...
    test = test_features[0, :].tolist()
    for i in range(1, len(test_features)):
        test.append(test_features[i, -1])
    test = np.array(test)
    ...
    for i in train_features:
        ...
        predict_train_input = InputData(idx=forecast_idx,
                                        features=i,
                                        target=i,
                                        task=task,
                                        data_type=DataTypesEnum.ts)
    ...
    for i in test_features:
        ...
        end_forecast = start_forecast + HORIZON
        forecast_idx = np.arange(start_forecast, end_forecast)
        predict_test_input = InputData(idx=forecast_idx,
                                       features=i,
                                        target=i,
                                        task=task,
                                        data_type=DataTypesEnum.ts)
    ...
    plt.title(f'FEDOT: {station}\nMAE={np.round(test_mae, 4)}, MSE={np.round(test_mse, 4)}')
    plt.tight_layout()
    plt.savefig(f'results/FEDOT/univariate/{station}.png', dpi=100)
    # plt.show()
    plt.close()
...
results_df.to_csv('results/fedot/univariate/metrics.csv', index=False)
