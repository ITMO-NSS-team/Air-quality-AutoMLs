...
for station in stations:
    ...
    train_prediction = naive_forecast(train_features)
    ...
    test_prediction = naive_forecast(test_features)
    ...
    plt.title(f'Naive forecast: {station}\nMAE={np.round(test_mae, 4)}, MSE={np.round(test_mse, 4)}')
    plt.tight_layout()
    plt.savefig(f'results/naive_forecast/{station}.png', dpi=100)
    # plt.show()
    plt.close()
...
results_df.to_csv('results/naive_forecast/metrics.csv', index=False)