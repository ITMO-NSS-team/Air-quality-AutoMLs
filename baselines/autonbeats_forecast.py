...
RESULTS_PATH = Path(__file__).parent.parent / 'baselines' / 'results' / 'autonbeats_forecast'
...
if __name__ == '__main__':
    ...
    for station in stations:
        ...
        for i in range(n_rolls):
            ...
            pred_chunk = y_hat_df[y_hat_df['unique_id'] == station]['AutoNBEATS'].values[:end_idx - start_idx]
        ...
        plt.plot(ds_vals[:500], preds[:500], label='Prediction', c='green', linewidth=1, linestyle='--')
        plt.legend()
        plt.xticks(rotation=15)
        plt.ylabel('MP10, µg/m³')
        plt.title(f'AutoNBEATS (rolling): {station}\nMAE={np.round(mae, 4)}, MSE={np.round(mse, 4)}')
        plt.tight_layout()
