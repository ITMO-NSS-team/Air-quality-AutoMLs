...
for station in stations_names:
    ...
    for set_type in ['train', 'test']:
        try:
            # Form file paths
            features_path = os.path.join(input_dir, f'{station}_{set_type}_features.npy')
            ...
            if not all(os.path.exists(p) for p in [features_path, target_path]):
                print(f"  - Skipping '{set_type}' set, as one or more files were not found.")
                continue
            ...
            print(f"  - An error occurred while processing '{set_type}' for station {station}: {e}")
