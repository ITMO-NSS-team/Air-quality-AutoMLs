...
combined_df.columns = ['Date', 'Time', 'Station', 'Code', 'Pollutant', 'Value', 'Unit', 'Type']
...
combined_df['Station'] = combined_df['Station'].replace('Vitória Centro', 'Vitoria-Centro')
combined_df['Station'] = combined_df['Station'].replace('Vitoria Centro', 'Vitoria-Centro')
combined_df['Station'] = combined_df['Station'].replace('Cariacica Vila Capixaba', 'Cariacica')
combined_df['Station'] = combined_df['Station'].replace('Enseada do Suá', 'Enseada do Sua')
...
single_station[f'24h_{pollutant}'] = single_station['Value'].rolling('24H', min_periods=1).mean()
...
plt.rcParams['figure.figsize'] = (10, 6)
ax = plot_risk_zones(pollutant, '24h')
plt.plot(single_station.index[-2000:-1000], single_station[f'24h_{pollutant}'][-2000:-1000])
plt.xticks(rotation=90)
plt.title(f'{pollutant} - {station_name}')
plt.ylabel('ug/m3')
plt.tight_layout()
