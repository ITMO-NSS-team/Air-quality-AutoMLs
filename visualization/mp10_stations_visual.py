...
for station in correct_stations:
    ...
    stations_target_dates[station] = f'{str(target_dates[0]).split(" ")[0]} - {str(target_dates[-1]).split(" ")[0]}'


    station_dataset['Value'][station_dataset['Value'] < 1] = 1
    station_dataset = station_dataset.set_index('Date').sort_index()
    station_dataset[f'24h_MP10'] = station_dataset['Value'].rolling('24H', min_periods=1).mean()
...
def save_mp10_station_existance():
    ...
    for i, station in enumerate(stations_pollutants_dict.keys()):
        for j, pollutant in enumerate(pollutants):
            ...
            ax.add_patch(rect)
            plt.text(j, i, symbol, ha='center', va='center', fontsize=14,
...
def save_test_dates():
    ...
    ax.axis('off')
    table_data = [['Station', 'Target Date Range']] + [[s, d] for s, d in zip(stations, dates)]
    table = ax.table(
        cellText=table_data,
        loc='center',
        cellLoc='left',
